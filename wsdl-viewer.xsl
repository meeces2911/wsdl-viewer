...
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
-->
<xsl:param name="ENABLE-W3C-VALIDATION-MODE" select="false()"/>
...
function adjustHeight(totalExpanded, srcHidden) {
	if (totalExpanded === 0 &amp;&amp; (!window.location.href.includes("#src") || srcHidden)) {
		$('#ops-content').height($('#services').height() - $('#about').outerHeight() - 24);
		$('#operations_title').css('display', 'none');
	} else {
		$('#ops-content').height('auto');
		$('#operations_title').css('display', 'initial');
	}
}
...
$(document).ready(function() {
	...
	$(".porttitle").click(function() {
		...
		adjustHeight(totalExpanded, srcHidden)
		
    });
	// Display the source code when we click on a link
	$(".local.src").click(function() {
		srcHidden = false;
		$("#src").show();
		adjustHeight(totalExpanded, srcHidden);
	});
	// Or if we are reloaded while looking at the src section
...
<xsl:template name="render.source-code-link">
	<xsl:if test="$ENABLE-SRC-CODE-PARAGRAPH and $ENABLE-LINK">
		<a class="local src" href="{concat('#', $SRC-PREFIX, generate-id(.))}">
...
<xsl:template match="ws:service|ws2:service" mode="service-start">
	...
	<xsl:apply-templates select="ws:port|ws2:endpoint" mode="service">
		<xsl:with-param name="PORT-CONTENT-POS" select="position()"/>
	</xsl:apply-templates>
</xsl:template>
...
<xsl:template match="ws:port" mode="service">
	<xsl:param name="PORT-CONTENT-POS"/>
	<xsl:variable name="content-visibility">
		<xsl:choose>
			<xsl:when test="$PORT-CONTENT-POS = 1">display: block</xsl:when>
			<xsl:otherwise>display: none</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	
	<xsl:variable name="collapsed-img">
		<xsl:if test="$ENABLE-W3C-VALIDATION-MODE = false">
	&lt;svg class="collapsed" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	...
	<xsl:variable name="expanded-img">
		<xsl:if test="$ENABLE-W3C-VALIDATION-MODE = false">
	&lt;svg class="expanded" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
		 width="16px" height="16px" viewBox="0 0 16 16" enable-background="new 0 0 16 16" xml:space="preserve"&gt;
    ...
    <div class="porttitle" id="{concat($PORT-TITLE-PREFIX, generate-id($port-type))}">
		<xsl:choose>
			<xsl:when test="$PORT-CONTENT-POS = 1">
    ...
    <div class="portcontent" id="{concat($PORT-CONTENT-PREFIX, generate-id($port-type))}" style="{$content-visibility}">
...
<div class="page">
    ...
    <a class="target" name="page.operations">
		<span id="operations_title">Operations:</span>
	</a>
	<ul>
...
<div class="page">
	<h2>
		<a class="target" name="page.src">WSDL source code</a>
	</h2>
	<a class="box hidetab top" href="#">Hide</a>
	<div class="box">
		<div class="xml-proc">
	...
	<a class="box hidetab" href="#">Hide</a>
</div>
</xsl:template>
...
<div class="page">
	<h2>
		<a class="target" name="page.about">About <em>wsdl-viewer.xsl</em></a>
	</h2>
